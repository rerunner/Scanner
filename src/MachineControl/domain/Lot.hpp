...
public:
  Lot();
  Lot(std::shared_ptr<cppkafka::Producer> newkafkaProducer);
  virtual ~Lot(){}

    void AddWafer(Verdi::Uuid wId);
    void RemoveWafer(Verdi::Uuid wId);
  ...
  //JSON boilerplate
  NLOHMANN_DEFINE_TYPE_INTRUSIVE(Lot, id_, parentId_, waferIds_)
};
